...
# The whitelist callbacks have to be enabled explicitly: they are what turns the
# hits of the whitelist rules into the exclusion list the blacklist rules
# consult, so without them the `Do Not Expect` below would hold for the wrong
# reason. Both are needed: RBL_ENVELOPE_WHITE serves the envelope part of a
# split rule (the from-IP check here) and RBL_CALLBACK_WHITE the message part.
# Same for FAKE_RBL_UNKNOWN_CHECK rather than FAKE_RBL_UNKNOWN: the rule has
# symbols_prefixes, so _CHECK is the name of its callback.
RBL FROM HIT WL
  Scan File  ${MESSAGE}  IP=4.3.2.4
  ...  Settings={symbols_enabled = [RBL_CALLBACK_WHITE, RBL_ENVELOPE_WHITE, FAKE_RBL_UNKNOWN_CHECK, FAKE_WL_RBL_UNKNOWN]}
  Do Not Expect Symbol  FAKE_RBL_CODE_2
