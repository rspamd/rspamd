...
Metric exporter allows for periodically pushing rspamd's internal statistics to an external monitoring system (currently just Graphite is supported). Metadata exporter is a flexible mechanism for conditionally pushing user-defined message metadata to an external system (current backends are Redis Pubsub & HTTP).
...
Various new rules to detect suspicious patterns; fixes to improve accuracy.
