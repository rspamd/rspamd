...
--[[
Example config for third and second level domains whitelist:
greylist {
...
local function greylist_set(task)
  ...
  if not is_whitelisted and whitelist_domains_map then
    ...
    if hostname and hostname ~= 'unknown' then
      ...
      local maxn = #h_split
      if maxn >= 2 then
        domain2l = h_split[maxn-1]..'.'..h_split[maxn]
        if maxn >= 3 then
          domain3l = h_split[maxn-2]..'.'..domain2l
        end
      end
      if (domain2l and whitelist_domains_map:get_key(domain2l)) or (domain3l and whitelist_domains_map:get_key(domain3l)) then
        is_whitelisted = 'meta'
...
if opts then
  ...
  if opts['whitelist_domains_url'] and #opts['whitelist_domains_url'] > 0 then
    whitelist_domains_map = rspamd_config:add_map ({
      url = opts['whitelist_domains_url'],
      type = 'map',
