...
static void
stream_unget (stream_t * stream, char c)
{
	g_assert (stream->buffer_pos > 0);
	stream->buffer_pos--;
	g_assert (stream->buffer[stream->buffer_pos] == c);
}


static int
lex_get (lex_t * lex, json_error_t * error)
{
...
lex_scan_string (lex_t * lex, json_error_t * error)
{
	 ...
	 */
	lex->value.string = g_malloc (lex->saved_text.length + 1);
	if (!lex->value.string) {
...
lex_scan_number (lex_t * lex, char c, json_error_t * error)
{
	...
	if (c != '.' && c != 'E' && c != 'e') {
		...
		g_assert (end == saved_text + lex->saved_text.length);
...
static int
lex_init (lex_t * lex, get_func get, eof_func eof, void *data)
{
