...
static int r_mark_regions(struct SN_env * z) {
    ...
    if (!(z->I[2] < 2)) return 0;
    {   int c4 = z->c;
        if (!(z->I[2] == z->I[1])) goto lab5;
        while(1) {
            if (z->c == z->l || z->p[z->c] != 'r') goto lab6;
            z->c++;
            break;
        lab6:
            {   int ret = skip_utf8(z->p, z->c, 0, z->l, 1);
...
extern struct SN_env * serbian_UTF_8_create_env(void) { return SN_create_env(0, 3); }
