...
Во-первых, sqlite крайне плохо работает при конкурентных запросах на запись: в этом случае производительность этой СУБД падает на несколько порядков. Во-вторых, довольно затруднительно обеспечивать репликацию и масштабирование базы данных, так как для этого нужны сторонние инструменты.
...
По умолчанию rspamd не разрешает модификаций в хранилище. Для того чтобы обучение было возможно, необходимо задать список доверенных ip адресов и/или сетей. Как правило, лучше всего разрешить запись только с локального адреса (127.0.0.1), так как fuzzy storage использует UDP, который не имеет никакой защиты от подделки IP адреса источника (что можно исправить путем настройки верификации обратного маршрута на маршрутизаторе, но это зачастую игнорируется системными администраторами):
...
Чтобы настроить транспортное шифрование, в первую очередь нужно создать ключевую пару сервера хранилища, что делается командой `rspamadm keypair -u`:
...
Данная команда создает **уникальную** пару ключей, в которой **публичный** ключ может быть скопирован вручную на клиенты (например, через ssh) или же опубликован неким безопасным способом (например, подписан gpg ключем или размещен на https сайте). Приватный ключ должен храниться в секрете.
...
В этом режиме клиенты, у которых нет валидного публичного ключа, не смогут обращаться к хранилищу.

### Проверка работы хранилища

Для проверки работы хранилища можно запустить команду `rspamadm control fuzzystat`:
...
Таким образом, вначале отображается общая статистика хранилища: количество сохраненных хешей, количество устаревших хешей. Также отображается статистика запросов по версиям протокола:
...
Плагин `fuzzy_check` используется процессами-сканерами для проверки писем и процессами-контроллерами для обучения хранилища. Плагин имеет следующие функции:

1. Обработка писем и создание хешей на их базе
2. Отправка запросов к хранилищу
3. Транспортное шифрование

Обучение осуществляется командой `rspamc fuzzy_add`:
...
Флаги позволяют хранить хеши различного происхождения в хранилище. Например, хеши спам-ловушек и хеши жалоб или же хеши писем из "белого" списка. Каждый флаг может быть ассоциирован с собственным символом и, соответственно, иметь свой вес при проверке письма:

<center><img class="img-responsive" src="/img/rspamd-fuzzy-4.png" width="75%"></center>

Для того чтобы rspamd мог соотносить флаги и символы, нужно настроить эти соответствия в секции `rule`. Приведем пример настройки данной секции:
...
**Важное замечание:** нет возможности изменить впоследствие этот параметр без потери всех данных в хранилище, так как все хеши создаются, используя конкретный алгоритм, а восстановление текста по хешу невозможно принципиально.
