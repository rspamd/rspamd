...
При создании собственного хранилища в первую очередь необходимо выбрать источники образцов спама для обучения. Основной принцип - использование спам-рассылок, которые приходят большому количеству получателей. Для решения данной задачи существует два основных подхода:
...
Первый метод довольно прост: давайте назначим каждой жалобе некий вес и будем просто прибавлять его к сохраненному значению хеша в базе при обучении. При проверке мы просто не будем учитывать хеши, вес которых меньше заданного порога. Например, если вес жалобы 1, а порог срабатывания 20, то для начала срабатывания хеша необходимо как миимум 20 жалоб пользователей. Кроме этого, при проверке rspamd не назначает максимальные очки при нахождении хеша с пороговым значением - это значение плавно растет от нуля до максимума в диапазоне весов хеша от порогового до удвоенного порогового значения (w .. 2*w).
