...
Зачастую бывает нужно собирать хеши из различных источников или же хранить локальную копию какого-либо внешнего хранилища. Для решения этой задачи в rspamd реализована поддержка репликации на уровне хранилища хешей. Данная возможность доступна с версии 1.3.

Схематично репликацию можно изобразить следующим образом:

<center><img class="img-responsive" src="/img/rspamd-fuzzy-5.png" width="75%"></center>

Передача хешей инициируется **мастером** репликации, который отправляет команды на обновление хешей (добавление, модификацию или же удаление) на все заданные слейвы. Таким образом, слейвы должны иметь возможность принимать соединение от мастера (что зачастую нужно учитывать при настройки firewall'а).

Для установки соединения слейв должен прослушивать тот же порт 11335 (по умолчанию), но по протоколу TCP. Синхронизация мастера и слейва осуществляется по протоколу HTTP с транспортным шифрованием HTTPCrypt. Для предотвращения повторных обновлений, а также неверных обновлений слейв проверяет версию обновления, и если она меньше или равно локальной, то такое обновление отвергается. В случае же, если мастер опережает слейв более чем на одну версию, в лог файле слейва будет информация вида:
...
В таком случае рекомендуется произвести процедуру синхронизации с нуля. На хосте мастере останавливается сервис rspamd, и делается дамп базы данных хешей (теоретически, rspamd можно не останавливать, но это менее надежно):

```
echo ".dump" | sqlite3 /var/lib/rspamd/fuzzy.db > fuzzy.sql
```

далее полученный `fuzzy.sql` копируется на все слейвы (rspamd на слейвах **должен быть** остановлен):

```
rm /var/lib/rspamd/fuzzy.db
sqlite3 /var/lib/rspamd/fuzzy.db < fuzzy.sql
chown _rspamd:_rspamd /var/lib/rspamd/fuzzy.db
```
