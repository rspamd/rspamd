...
Так как плагин `fuzzy_check` отвечает также за обучение, то именно в его конфигурации задается скрипт, определяющий, какие сообщения будут поступать на обучение, а какие нет. Такой скрипт представляет собой lua функцию с одним аргументом типа [`rspamd_task`](/doc/lua/task.html) и возвращает либо булево значение: `true` - обучать и `false` - не обучать, либо пару - булево значение и новый флаг, если в ходе работы скрипта выяснилось, что необходимо изменить исходный флаг обучения. Для задания условного скрипта служит параметр `learn_condition`, сам же скрипт удобнее всего записывать в multiline строке, поддерживаемой `UCL`:
...
Приведем практические примеры полезных скриптов. Например, часто нужно не давать проводить обучение с некоторых доменов. Для этого можно задать такой скрипт:
...
Также, бывает полезным распеределять обучение хешей по различным флагам в соответствии с их источником, который можно закодировать, например, в заголовке `X-Source`. Допустим, мы хотим иметь такое соответствие флагов и источников:
