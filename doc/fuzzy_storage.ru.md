...
Нечеткие (fuzzy) хеши используются для нахождения **похожих** писем - то есть, при помощи такого метода можно найти сообщения, которые имеют такой же или незначительно измененный текст. Данная технология хорошо подходит для блокировки спам-рассылок, которые отправляются многим пользователям одновременно. Так как хеш функция однонаправлена, то восстановить исходный текст, имея только хеш, невозможно, что позволяет отправлять запросы на сторонние хранилища хешей без риска раскрытия информации.

Хочется отметить, что fuzzy хеши работают не только для текстовых данных, но и для картинок и других вложений в письмах. Однако в таком случае пропадает элемент нечеткой логики, и rspamd ищет точные соответствия для подобных объектов.

Далее в данной статье я подробно рассмотрю, каким образом создать собственное хранилище хешей, каким образом наполнять его и как использовать его для проверки писем пользователей.

## Шаг 1: выбор источников хешей

При создании собственного хранилища первым вопросом, которым стоит задаться, является вопрос выбора, откуда обучать хранилище примерами спама. Основной принцип в данной задаче - искать спам рассылки, которые приходят многим вашим пользователям. Используется два основных подхода для решения данной задачи:
...
Пользователи любят жаловаться на спам, поэтому неплохо обеспечить механизм передачи этих жалоб в хранилище хешей. К сожалению, пользователи также любят жаловаться на легитимные письма, на которые они сами подписались, - рассылки магазинов, уведомления от систем бронирования билетов и даже на личные письма, которые им по каким-то причинам не нравятся. Многие пользователи просто не видят разницу между кнопкой "удалить" и "пожаловаться на спам". Возможно, хорошей идеей будет спросить у пользователя некую дополнительную информацию о жалобе, например, почему он считает, что это письмо спам.

В rspamd есть два метода, помогающих работать с жалобами пользователей и отсеивать "ложные" срабатывания:

1. Вес хеша.
2. Фильтры обучения.

Первый метод довольно прост: давайте назначим каждой жалобе некий вес и будем просто прибавлять его к сохраненному значению хеша в базе при обучении. При проверке мы просто не будем учитывать хеши, чей вес меньше заданного порога. Например, если мы назначили вес жалобы в единицу, а порог срабатывания в 20, тогда для начала срабатывания хеша необходимо как миимум 20 жалоб пользователей. Кроме этого, при проверке rspamd не назначает максимальные очки при нахождении хеша с пороговым значением - это значение плавно растет от нуля до максимума в диапазоне весов хеша от порогового до удвоенного порогового значения (w .. 2*w).
...
В данном методе используется тот факт, что спамеры рассылают свои письма сразу многим получателям, поэтому неплохо было бы иметь такие почтовые адреса, на которые не приходит легитимной почты, зато приходит много спама. Основной подход к созданию таких адресов состоит в том, чтобы "засветить" этот адрес в базах спамеров, не показывая его легитимным пользователям. Для этого можно применять, скажем, элемент *iframe* на достаточно популярном веб-сайте, который не отображается пользователям (имеет свойство hidden или нулевой видимый размер), но показывает ботам различные email адреса. В последнее время такой метод не очень эффективен, так как спамеры также научились обходить такие адреса стороной. Другой возможный способ создания ловушки - это поиск доменов, бывших ранее популярными, но не работающих в настоящее время (адреса с этих доменов находятся во многих спамерских базах). При таком подходе зачастую нужны фильтры, так как высока вероятность того, что в "черный" список занесутся хорошие письма, например, от социальных сетей или служб рассылки.

В целом, создание собственных ловушек оправдано только в случае большой почтовой системы, так как это может быть затратно как в плане трудности сопровождения, так и в виде прямых материальных расходов на скупку доменов.

## Шаг 2: Настройка хранилища

В данной главе я рассмотрю основные настройки хранилища и то, каким образом оптимизировать работу хранилища хешей. В первую очередь, хотелось бы сделать одно важное замечание: хранилище не работает с письмами, а с готовыми хешами. То есть, чтобы преобразовать письмо в его хеш нужен отдельный процесс сканера или котроллера:

<center><img class="img-responsive" src="/img/rspamd-fuzzy-2.png" width="75%"></center>

Тем не менее, хранилище отвечает за следующие задачи:

1. Хранение и проверка хешей
2. Транспортное шифрование протокола
3. Устаревание хешей
4. Определяет политику доступа (как на запись, так и на чтение)
5. Выполняет синхронизацию (с версии 1.3)
